    ...
    for bucket in bucket_resource.buckets.all():
        if bucket.name == 'olx-latam-bi':
            for obj in bucket.objects.filter(Prefix=table_bucket_path):
                ...
                #print("Date from parameter",date_from, date_from+timedelta(days=200))
                if (obj.last_modified.date() >= date_from   and  obj.last_modified.date()  < date_from+timedelta(days=365)):                        
...
def main():
    ...
    if date_from == '%' and date_to == '%':
        date_from = str(datetime.date.today() - timedelta(days=5))
    ...
    # Get S3 Objects:
    reservoir_resource = connect_to_bucket("arn:aws:iam::657245296933:role/latam-bi-redshift","arn:aws:iam::918620439975:role/Core-Data-Role-Users-Reservoir-latam-bi")
    bucket_resource = connect_to_bucket("arn:aws:iam::657245296933:role/latam-bi-redshift")

    # Get tables to check posibles missing files
    working_tables = get_tables_to_check(configfilename)
    #print(working_tables)

    # Find for missing files for each table
    missing = []
    for row in working_tables:
        #print(row[0][33:],  row[1][18:], date_from_date, date_to_date)
        missing = missing + get_missing_files(reservoir_resource, bucket_resource, row[0][33:], row[1][18:], date_from_date, date_to_date)


    if len(missing) > 0:  
        #print("HAY FALTANTES")
        with open("{}/missing_files.csv".format(workspace_path), "w") as f:
            f.write( '\n'.join( missing ) )        
    
    # Clean the temporary working folder
    #os.system('rm /home/ubuntu/transformaciones/input/temp/PAD/tmpfiles/*')        
